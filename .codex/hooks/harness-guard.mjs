...
if (tool === "Edit" || tool === "Write" || tool === "MultiEdit") {
  ...
  if (/playwright-driver\.ts$/i.test(path) && /waitfortimeout/i.test(code)) {
    warn(
      "⚠ driver 纪律(仅警告): 新增了 waitForTimeout 死等。请改用事件式等待(waitForItemFormReady / settleRecalc)，参见 line-item-entry 技能。",
    );
  }
  ...
  if (/(llm-extractor|pdf-text)\.ts$/i.test(path)) {
    warn(
      "🔗 提醒(仅警告): 改动了抽取核心。请同步更新 test/fixtures/pdf-extract 的 golden fixture 并复测准确率，参见 golden-fixture 技能。",
    );
  }
...
if (tool === "Bash") {
  ...
  if (/git\s+commit/.test(cmd)) {
    warn(
      "✅ 提醒(仅警告): 提交前请确认 `npm run verify` 通过(typecheck + lint + format:check + test)。",
    );
  }
  process.exit(0);
}
